...
When running through the doyaken CLI, verification gates (build, lint, format, test) run automatically after key phases. Each phase has a configurable retry budget — if gates fail and retries remain, the phase re-runs with the error output in context.

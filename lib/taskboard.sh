...
#
# Works with both .ai-agent/ (new) and .claude/ (legacy) structures.
#
set -euo pipefail

# Configuration
AI_AGENT_HOME="${AI_AGENT_HOME:-$HOME/.ai-agent}"
AI_AGENT_PROJECT="${AI_AGENT_PROJECT:-$(pwd)}"

# Detect project structure
if [ -d "$AI_AGENT_PROJECT/.ai-agent/tasks" ]; then
  TASKS_DIR="$AI_AGENT_PROJECT/.ai-agent/tasks"
  TASK_PATH_PREFIX=".ai-agent/tasks"
elif [ -d "$AI_AGENT_PROJECT/.claude/tasks" ]; then
  TASKS_DIR="$AI_AGENT_PROJECT/.claude/tasks"
  TASK_PATH_PREFIX=".claude/tasks"
else
  echo "Error: No task directory found in $AI_AGENT_PROJECT"
  exit 1
fi

OUTPUT_FILE="$AI_AGENT_PROJECT/TASKBOARD.md"
...
> Auto-generated by `ai-agent tasks` or `lib/taskboard.sh`
...
# Create a new task
ai-agent tasks new "My task title"
